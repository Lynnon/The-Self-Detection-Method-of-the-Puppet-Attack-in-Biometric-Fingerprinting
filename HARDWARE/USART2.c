...
void Usart2_Init(unsigned int bound2)
{
			...
			USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;//无硬件数据
...
#endif        
    USART_Init(USART2, &USART_InitStructure);                                      //设置串口
...
{  
	if(USART_GetITStatus(USART2, USART_IT_TXE) != RESET) //USART_IT_TC这里也可替换
			{    
