...
static u8 *JudgeStr(u16 waittime)
{
  ...
  str[6] = 0x07;
  str[7] = '\0';
	//u2_printf("%d%d%d%d%d%d%d%d",str[0],str[1],str[2],str[3],str[4],str[5],str[6],str[7]);
  USART1_RX_STA = 0;
  while(--waittime)
  {
    delay_ms(1);
...
void Add_FR(void)
{
  ...
  while(1)
  {
    ...
    case 2:
      ...
      if(ensure == 0x00)
      {
        OLED_ShowCH(0, 4, "    对比成功    ");
        processnum = 3; //跳到第四步
      }
      else
      {
        OLED_ShowCH(0, 4, "    对比失败    ");
...
//	
//录入图像 PS_UpImage
//功能:返回应答包后，将ImageBuffer中的数据发送出去。
//模块返回确认字
u8 PS_UpImage(void)
{
  u8  ensure;
...
//			t=0;
//			
//      int length=strlen((const char*)USART1_RX_BUF);
//			while((USART2->SR&0X40)==0); 
//			for(int i = 0;i < length;i ++)            
//			{			
//				USART2->DR = USART1_RX_BUF[i];
//				while((USART2->SR&0X40)==0);	
//			}	
//			
//    }
...
unsigned char hex_to_char(unsigned char hex)
{
    if((hex>=0x00)&&(hex<=0x09))
        hex = hex+0x30;
    else if((hex>=0x0A)&&(hex<=0x0F)) // capital
        hex = 'A' + (hex - 0x0A);
...
void Sent_data(void)
{
	...
	//int length=strlen((const char*)USART1_RX_BUF);
	//int length=sizeof(USART1_RX_BUF)/8;
	//int leng=sizeof(USART1_RX_BUF);
	
		//	for(int i=0;i<length;i++)
//	{
//		unsigned char high=(USART1_RX_BUF[i]&0xf0)>>4;
//		unsigned char low=USART1_RX_BUF[i]&0x0f;
//		USART1_RX_BUF[i]=hex_to_char(USART1_RX_BUF[i]);
//	}
		
	
	for(int j=0;300*j<9000;j++)
	{
		
		for(int i=0;i<300;i++)
		{
			unsigned char high=(USART1_RX_BUF[i+300*j]&0xf0)>>4;
			unsigned char low=USART1_RX_BUF[i+300*j]&0x0f;
			arr[3*i]=hex_to_char(high);
			arr[3*i+1]=hex_to_char(low);
			arr[3*i+2]=' ';
		}
	...
	for(int j=0;300*j<9000;j++)
	{
		...
		for(int i=0;i<300;i++)
		{
			unsigned char high=(USART1_2_RX_BUF[i+300*j]&0xf0)>>4;
			unsigned char low=USART1_2_RX_BUF[i+300*j]&0x0f;
			arr[3*i]=hex_to_char(high);
			arr[3*i+1]=hex_to_char(low);
			arr[3*i+2]=' ';
		}
